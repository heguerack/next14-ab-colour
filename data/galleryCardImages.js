// here we are grabbing the first object of pretty much each set of images from sevices and kind of painting
export const galleryCards = [
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1687128428/gallery/interiors/built-ins/moha-beerspaw/PXL_20220622_191517328_shmefp.jpg',
    alt: 'builtin with a chimney in the middle  before painted',
    title: 'Built-in Painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797221/gallery/drywall/ceiling-texturing-showcase/knockdown-ceiling-textuting_ypz2aw.webp',
    alt: 'knockdown ceiling texture sample from Alberta Colour Painting in Calgary',
    title: 'ceiling Texturing',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797223/gallery/interiors/commercial/commercial-painting-project-downtown-paint-prep_rdhygl.webp',
    alt: 'commercial painting - long wall being divided with frog tape, so that when painted it comes in 3 colours for diferent shapes ',
    title: 'Commercial Painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1687019711/gallery/drywall/skyview/garage-drywall-repair-calgary_valeh0.jpg',
    alt: 'Calgary garage drywall repair - A 2 feet by 3 feet hole before fixed',
    title: 'drywall $ Mud',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797204/gallery/exteriors/siding/blue-siding/exterior-siding-being-painted_d98zgd.jpg',
    alt: 'Exterior siding being painted by Calgary exterior painters from Alberta Colour Painting',
    title: 'exterior Painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797184/gallery/interiors/garages/auburn%20garage/20201113_110122_ustpqw.jpg',
    alt: 'Garage with bare drywall in Calgary, Alberta, ready to be primed',
    title: 'Garage Painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797202/gallery/interiors/cabinets/donalson/20200814_133048_ru924w.jpg',
    alt: 'Kitchen cabinets to be spray painted in Calgary, Alberta, showing doors and drawers by stove and microwave',
    title: 'Cabinet painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797205/gallery/interiors/ceilings/marc-ceilings/IMG-20210103-WA0032_gwbxw9.jpg',
    alt: 'interior ceiling prepration for spray painting in central Calgary',
    title: 'Interior Painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797230/gallery/drywall/texture-removal-process-calgary_zhmkvb.webp',
    alt: 'A Calgary Popcorn ceiling being removed by hand with a putty knife. All you can see from the person doin t is his or her arm',
    title: 'Popcorn Removal',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797201/gallery/interiors/railings/janikke/20210220_093235_chhotc.jpg',
    alt: 'A brown railing prior to being paint sprayed',
    title: 'Railing Painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797205/gallery/exteriors/siding/blue-siding/20200825_160214_p1njz1.jpg',
    alt: 'exterior siding of a house  being prepared for exterior painting , garage at the back, the actual colour is light brown, in Calgary',
    title: 'Siding Painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1687186194/gallery/interiors/full-interiors/moha-unknown/IMG-20220916-WA0023_b6vdjx.jpg',
    alt: 'Trim prior to being sprayed, bondo has been apllied to eliminate dents and scratches. bondo is red colou easy to sand, sand like dust.',
    title: 'Trim Painting',
  },
  {
    url: 'https://res.cloudinary.com/alberta-colour-painting/image/upload/v1672797193/gallery/interiors/walls/chris-friend-patches/IMG-20210308-WA0000_n33j46.jpg',
    alt: 'Interior wall with many patches in a standard bedroom in Calgary, Alberta',
    title: 'Wall Painting',
  },
]
