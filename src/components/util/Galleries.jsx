...
export default function Galleries({ bg }) {
  ...
  const allImages = [
    ...
    garageImages,
    cabinetImages,
    // homeGalleryImages, not included as this is the main gallery
    interiorImages,
    popcornImages,
    railingImages,
    sidingImages,
  ...
  const getImages = (index) => {
    ...
    const filteredImages = allImages[index]
  ...
  return (
    <div className={'pb-8 sm:pb-2'}>
        ...
        {galleryCards.map((card, index) => (
          <div
            ...
            <button
              ...
              aria-label={`Open gallery for ${card.title}`}>
              <Image
                src={card.url}
                alt={card.alt}
      ...
      {showSliderFullScreen && (
        <div
          className='fixed top-0 left-0 bg-[rgba(0,0,0,.9)] z-50 flex items-center justify-center h-full w-full'
          role='dialog'
          aria-modal='true'
